...
async function main() {
  ...
  await prisma.user.upsert({
    where: { email: 'admin@rekayasa.com' },
    update: {
      password: password,
    },
    create: {
  ...
  // 5. Create Suppliers from Legacy Data
  ...
  for (const s of legacySuppliers) {
    // Generate simple code for seeding
  ...
  // 6. Create Product Categories & Products from Legacy Data
  ...
  // 7. Create Banks & Internal Accounts from Legacy Data
  ...
  for (const acc of legacyAccounts) {
    if (!acc.bankId) continue; // Skip if bank reference missing from CSV
