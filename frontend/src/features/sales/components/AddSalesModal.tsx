...
import { Plus, Trash2, Save, Loader2, ShoppingCart, Calendar as CalendarIcon } from 'lucide-react';
...
interface SalesRow {
  ...
  colM: string | number; colN: string | number; colO: string | number;
  colP: string | number; colQ: string | number; colR: string | number;
  colS: string | number; colT: string | number; colU: string | number;
  colV: string | number; colW: string | number; colX: string | number;
  colZ: string | number; colAA: string | number; colAB: string | number;
  colAC: string | number; colAD: string;
}
...
const NUMERIC_COLS: (keyof SalesRow)[] = [
...
export default function AddSalesModal({ open, onOpenChange, onSuccess }: AddSalesModalProps) {
  ...
  useEffect(() => {
    if (open) {
      setRows(Array(5).fill(null).map(() => ({
        ...
        colP: '', colQ: '', colR: '', colS: '', colT: '',
        colU: '', colV: '', colW: '', colX: '', colZ: '',
        colAA: '', colAB: '', colAC: '', colAD: ''
      } as SalesRow)));
    }
  }, [open]);
  ...
  const handlePaste = (e: React.ClipboardEvent, rowIndex: number, colKey: keyof SalesRow) => {
  ...
  const handleKeyDown = (e: React.KeyboardEvent, rowIndex: number, colKey: keyof SalesRow) => {
        ...
        <DialogFooter className="flex flex-col sm:flex-col gap-0 p-0 border-t border-primary/5 bg-primary/[0.02]">
          ...
          <div className="px-8 py-4 flex items-center justify-end w-full gap-3">
            ...
            <Button onClick={handleSave} disabled={loading} className="rounded-xl bg-primary hover:bg-primary/90 text-white font-black uppercase tracking-widest text-[11px] px-8 gap-2 shadow-premium cursor-pointer">
              {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save size={16} />} {loading ? "Saving..." : "Save Sales Records"}
            </Button>
          </div>
        </DialogFooter>
      </DialogContent>
