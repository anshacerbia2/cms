...
export function ApPpnNonWapuTab() {
  ...
  const { data: allData = [], isLoading, refetch } = getAllApTaxLedger({ type: 'NON_WAPU' });
    ...
    <div className="space-y-6">
      <div className="flex flex-col xl:flex-row items-stretch xl:items-center gap-4 bg-white/50 p-2 rounded-2xl border border-primary/5 backdrop-blur-sm">
        <div className="relative flex-1 w-full">
          ...
          <Input 
            placeholder="Search PPN Non-WAPU (Client, No Faktur, Reference)..." 
            ...
            <TableHeader className="bg-slate-50/50">
              <TableRow className="hover:bg-transparent border-primary/5 whitespace-nowrap">
                ...
                <TableHead className="text-right w-36">
                  <div className="flex items-center justify-end gap-1">
                    Non WAPU
                    <ExcelColumnFilter columnKey="colI" label="Non WAPU" data={getCascadingData("colI")} activeFilters={filters["colI"]} onFilterChange={(v) => { setFilters(p => ({...p, colI: v})); setPage(1); }} currentSort={sort} onSort={(d) => setSort({key: "colI", direction: d})} />
                  </div>
                </TableHead>
                <TableHead className="text-right w-36">
                  <div className="flex items-center justify-end gap-1">
                    Masukan
                    <ExcelColumnFilter columnKey="colJ" label="Masukan" data={getCascadingData("colJ")} activeFilters={filters["colJ"]} onFilterChange={(v) => { setFilters(p => ({...p, colJ: v})); setPage(1); }} currentSort={sort} onSort={(d) => setSort({key: "colJ", direction: d})} />
                  </div>
                </TableHead>
                <TableHead className="text-right w-36">
                  <div className="flex items-center justify-end gap-1">
                    Balance
                    <ExcelColumnFilter columnKey="colK" label="Balance" data={getCascadingData("colK")} activeFilters={filters["colK"]} onFilterChange={(v) => { setFilters(p => ({...p, colK: v})); setPage(1); }} currentSort={sort} onSort={(d) => setSort({key: "colK", direction: d})} />
                  </div>
                </TableHead>
                <TableHead className="w-48 px-4">
                  <div className="flex items-center gap-1">
                ...
                <TableRow>
                  <TableCell colSpan={15} className="h-96 text-center">
                    <div className="flex flex-col items-center justify-center gap-4">
                      ...
                      <p className="text-[10px] font-black uppercase tracking-[0.4em] text-primary/40">Fetching PPN Non-WAPU Details...</p>
                    </div>
                  </TableCell>
                </TableRow>
                ...
                <>
                  {paginatedData.map((row: any) => (
                    <TableRow key={row.id} className="hover:bg-slate-50/50 transition-colors whitespace-nowrap group">
                      ...
                      {/* <TableCell className="py-3 px-4 text-[12px] text-primary/60 border-r border-primary/5">{row.colN || '-'}</TableCell> */}
                  ...
                  <TableRow className="bg-secondary/5 border-t-2 border-secondary/30 hover:bg-secondary/5 transition-none font-bold whitespace-nowrap">
                    ...
                    <TableCell className={`text-right ${Number(subtotalTotals.colJ.toString()) < 0 ? "text-rose-600" : "text-primary/80"}`}>
                  ...
                  <TableRow className="bg-secondary/10 border-t border-secondary/30 hover:bg-secondary/10 transition-none font-bold whitespace-nowrap">
                    ...
                    <TableCell className={`text-right ${Number(grandTotals.colJ.toString()) < 0 ? "text-rose-600" : "text-primary/80"}`}>
