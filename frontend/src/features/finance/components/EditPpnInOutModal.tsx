...
export default function EditPpnInOutModal({ open, onOpenChange, recordId, onSuccess }: EditPpnInOutModalProps) {
  const { updatePpnInOut, getPpnInOutById } = usePpnInOut();
  const [loading, setLoading] = useState(false);

  const { data: record, isLoading: isFetching } = getPpnInOutById(open ? recordId : null);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success('PPN In/Out record updated successfully.');
      onSuccess();
      onOpenChange(false);
    } catch (error: any) {
