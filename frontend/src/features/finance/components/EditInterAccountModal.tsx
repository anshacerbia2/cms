...
export default function EditInterAccountModal({ open, onOpenChange, recordId, onSuccess }: EditInterAccountModalProps) {
  ...
  const { data: record, isLoading: isFetching } = getInterAccountById(open ? recordId : null);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success('Inter-Account record updated successfully.');
      onSuccess();
      onOpenChange(false);
    } catch (error: any) {
      toast.error(error?.response?.data?.message || 'Failed to update Inter-Account record.');
    } finally {
      setLoading(false);
    }
  };
